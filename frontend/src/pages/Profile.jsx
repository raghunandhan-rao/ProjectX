...
// // //       {/* 2. Add a Link to the Discover page */}
// // //       <Link to="/discover" className={styles.submitButton} style={{marginBottom: '20px', backgroundColor: '#17a2b8'}}>
// // //         Find Friends
// // //       </Link>
...
// //       {/* Updated to use optional chaining for safer data access */}
// //       <div style={{ textAlign: 'left', display: 'inline-block' }}>
// //           <p><strong>Name:</strong> {userProfile?.username || 'N/A'}</p>
// //           <p><strong>Email:</strong> {userProfile?.email || 'N/A'}</p>
// //           <p><strong>Star Rating:</strong> {userProfile?.starRating || 0} ★</p> 
// //       </div>
...
// function Profile() {
//   // The redirection logic has been removed from this component
...
//   // The loading state is now handled by ProtectedRoute, so this check is simpler
//   if (!userProfile) {
...
function Profile() {
  // Redirection logic is no longer needed here; it's handled globally.
  const { userProfile, logout } = useAuth();
  const navigate = useNavigate();
  ...
  // The loading state is handled by ProtectedRoute, so this component
  // will only render when userProfile is available.
  if (!userProfile) {
    return null; 
  }
